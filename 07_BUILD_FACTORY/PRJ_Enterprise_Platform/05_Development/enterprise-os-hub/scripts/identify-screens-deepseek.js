/**
 * AI Vision Screen Identifier using DeepSeek
 * Drop-in replacement for identify-screens-gemini.js
 *
 * Usage: node scripts/identify-screens-deepseek.js [screens_dir] [output_file]
 *
 * Setup:
 *   1. Set env: DEEPSEEK_API_KEY=your_key_here
 *   2. Run: node scripts/identify-screens-deepseek.js
 *
...
function callDeepSeekAPI(apiKey, imageBase64, filename) {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (!text) {
            reject(new Error(`No content in DeepSeek response. Raw: ${data.substring(0, 300)}`));
            return;
          }
...
async function processScreen(apiKey, filePath, retries = 0) {
  ...
  try {
    const result = await callDeepSeekAPI(apiKey, imageBase64, filename);
...
async function main() {
  ...
  if (!screensDir || !fs.existsSync(screensDir)) {
    ...
    console.error('Usage: node scripts/identify-screens-deepseek.js [screens_dir] [output_file]');
    process.exit(1);
  }
  ...
  console.log('\n=== DEEPSEEK VISION SCREEN IDENTIFIER ===');
