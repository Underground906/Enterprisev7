#!/usr/bin/env python3
"""Run Phases 2-5 for Core Dashboard Builder (light screens only)."""
...
kit_name = "Core Dashboard Builder"
kit_dir = KIT_SCREENS_DIR / kit_name

# --- PHASE 2: Structure Analysis ---
print("Phase 2: Loading FIGMA_FULL.json...")
...
# Filter: LIGHT only, desktop only (user requested light screens)
filtered = []
for s in all_screens:
    name_lower = s["name"].lower()
    # Must contain "light" (this kit uses Name/Device/Light|Dark naming)
    if "light" not in name_lower:
        continue
    # Desktop only
...
print(f"Phase 2: {len(all_screens)} total screens, {len(filtered)} light/desktop")
